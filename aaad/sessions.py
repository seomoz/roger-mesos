...
def load_token(token):
    ...
    try:
        data = login_serializer.loads(token, max_age=SESSION_MAXAGE_SECONDS)
    except BadSignature, e:
        # bad signature, let's log and return None
        logger.warn('Signature validation failed for token {}'.format(token))
        return None
